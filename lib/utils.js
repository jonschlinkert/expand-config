...
utils.reserved = utils.taskKeys
  .concat(utils.targetKeys)
  .concat(utils.optsKeys);

function nextId(key, num) {
  return key + '_' + (num++);
}
...
function isTaskKey(key) {
  return has(utils.taskKeys, key) && !isTargetKey(key);
}

function isTargetKey(key) {
  return has(utils.targetKeys, key);
}

function isOptionsKey(key) {
  return has(utils.optsKeys, key);
}

function isOptions(key) {
  return key && (key === 'options' || isOptionsKey(key));
}
...
function isType(config, key) {
  if (isOptions(key)) {
    return 'options';
  }
  if (isTask(config, key)) {
    return 'task';
  }
  if (isTarget(config, key)) {
    return 'target';
  }
  if (isNode(config, key)) {
    return 'node';
  }
  return 'config';
}
