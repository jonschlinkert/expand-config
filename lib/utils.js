...
lazy.replaceExt = function replaceExt(fp, options) {
  var opts = lazy.extend({extDot: 'first'}, options);
  var re = {first: /(\.[^\/]*)?$/, last: /(\.[^\/\.]*)?$/};
  return fp.replace(re[opts.extDot], opts.ext);
};

lazy.hasValues = function hasValues(obj, keys) {
  var res = lazy.pick(obj, keys);
...
lazy.contains = function contains(arr, val) {
